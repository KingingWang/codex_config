...
pub fn show(app: &mut App, ui: &mut Ui, _ctx: &Context) {
            ...
            .map(|list| {
                list.iter()
                    .map(|model| {
                        ...
                        (slug.clone(), format!("{} ({})", catalog::display_name(model), slug))
                    })
                    .collect()
            })
            .unwrap_or_default();
    ...
    widgets::card(ui, |ui| {
        ...
        ui.horizontal(|ui| {
            ...
            if widgets::text_field_w(ui, "new-profile-name", &mut buffer, "新配置档的名字，例如 work", 280.0) {
    ...
    for name in names {
        ...
        widgets::card(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                ...
                if is_active {
                    widgets::badge(ui, &format!("{} 当前启用", icons::CHECK), theme::OK, theme::OK_WEAK);
                }
            ...
            let toggle_label = if selected { format!("{} 收起", icons::CARET_DOWN) } else { format!("{} 编辑内容", icons::CARET_RIGHT) };
...
fn profile_summary(app: &App, name: &str) -> String {
    ...
    let at = |key: &str| doc.config.str_at(&["profiles", name, key]).unwrap_or_default();
...
fn profile_editor(app: &mut App, ui: &mut Ui, provider_options: &[(String, String)], model_options: &[(String, String)]) {
    let Some(mut editor) = app.profile_editor.clone() else { return };
    ...
    widgets::card(ui, |ui| {
        ...
        if widgets::field(
            ...
            |ui| widgets::string_dropdown(ui, "profile-model", &mut value, model_options, "（沿用全局）"),
        ) {
            editor.model = value;
            changed = true;
        }
        ...
        if widgets::field(
            ...
            |ui| widgets::string_dropdown(ui, "profile-provider", &mut value, provider_options, "（沿用全局）"),
        ) {
        ...
        if widgets::field(
            ...
            |ui| widgets::choice_dropdown(ui, "profile-effort", &mut value, REASONING_EFFORTS, true),
        ) {
        ...
        if widgets::field(
            ...
            |ui| widgets::choice_dropdown(ui, "profile-approval", &mut value, APPROVAL_POLICIES, true),
        ) {
        ...
        if widgets::field(
            ...
            |ui| widgets::choice_dropdown(ui, "profile-personality", &mut value, PERSONALITIES, true),
        ) {
