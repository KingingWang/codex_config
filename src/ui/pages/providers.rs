...
pub fn show(app: &mut App, ui: &mut Ui, ctx: &Context) {
    ...
    if rows.is_empty() {
        widgets::card(ui, |ui| {
            ...
            ui.horizontal(|ui| {
                if widgets::primary_button(ui, &format!("{} 添加服务商（选模板）", icons::ADD))
                    .clicked()
                {
            ...
            ScrollArea::vertical()
                ...
                .show(ui, |ui| {
                    if filtered.is_empty() {
                        widgets::hint(ui, "没有找到服务商，试试其他名称。");
                    }
                    for row in &filtered {
    ...
    match action {
        ...
        ProviderAction::Rename(id) => {
            app.dialog = Some(Dialog::RenameProvider {
                old: id,
                buffer: String::new(),
            });
        }
...
fn editor(
    ...
    ui.label(
        RichText::new(format!(
            "当前服务商（config.toml 里的 model_provider）：{}",
    ...
    egui::CollapsingHeader::new(format!("通信协议 · {}（按需更改）", editor.wire_api.label()))
        .id_salt(("provider-protocol", &row.id)).show(ui, |ui| {
    widgets::section(ui, icons::PROTOCOL, "通信协议", "按服务商文档选择；不确定时保留模板值", |ui| {
        ...
        |ui| {
            if widgets::field(
                ...
                FieldSpec::new(
                    "接口地址",
                    "base_url",
                    "模型服务的根地址。中转站一般长这样：https://xxx.com/v1。本地服务是 http://localhost:11434/v1。",
                ),
                |ui| {
                    widgets::mono_field(
                        ui,
            ...
            if query_open > 0 {
                ui.label(
                    RichText::new("URL 查询参数 query_params")
                        .size(12.5)
                        .strong()
                        .color(theme::TEXT),
                );
    ...
    widgets::section(
        ...
        "Codex 用什么凭证访问这个服务商",
        |ui| {
            egui::ComboBox::from_id_salt("provider-auth-mode")
                .selected_text(editor.auth_mode.label())
                .width(ui.available_width())
                .show_ui(ui, |ui| {
                    for mode in AuthMode::ALL {
                        if ui
                            .selectable_value(&mut editor.auth_mode, mode, mode.label())
            ...
            match editor.auth_mode {
                AuthMode::EnvKey => {
                    if widgets::field(
                        ...
                        FieldSpec::new(
                            "环境变量名",
                            "env_key",
                            "Codex 启动时会读这个环境变量当 API Key。先在终端里 export 好，例如 export OPENAI_API_KEY=sk-xxx。",
                        ),
                        |ui| {
                            widgets::mono_field(
                                ui,
                    ...
                    if widgets::field(
                        ...
                        FieldSpec::new(
                            ...
                            "可选。写给未来的自己看：这个 key 去哪里申请。",
                        ),
                        |ui| {
                            widgets::text_field(
                                ui,
                                "p-envinstr",
                                &mut editor.env_key_instructions,
                                "例如：去 platform.openai.com 申请",
                            )
                        },
                    ) {
                        changed = true;
                        ...
                        match std::env::var(&key) {
                            ...
                            _ => {
                                widgets::note(
                                    ui,
                                    &format!(
                                        "环境变量 {key} 现在没有值。请在终端里执行：export {key}=你的密钥，然后重新启动 Codex。\n（这里显示的是本工具启动时读到的环境，如果你刚在别的窗口 export 过，重开一下本工具就能看到。）"
                                    ),
                                    theme::WARN,
                                );
                            }
                ...
                AuthMode::Bearer => {
                    if widgets::field(
                        ...
                        FieldSpec::new(
                            ...
                            "直接写在配置文件里的密钥。方便，但任何能读到这个文件的人都能拿到它。",
                        ),
                    ...
                    widgets::note(
                        ui,
                        "这个值会以明文保存在 config.toml 里。分享配置或提交到 git 前记得删掉。",
                        theme::WARN,
                    );
                }
                AuthMode::CodexLogin => {
                ...
                AuthMode::None => {
                    widgets::hint(
                        ui,
                        "不会写入任何认证字段，适合本地服务（Ollama / LM Studio）。",
                    );
                }
            }
        },
    ...
    let advanced_open = ui
        .horizontal(|ui| {
            let arrow = if editor.open_advanced {
                icons::CARET_DOWN
            } else {
                icons::CARET_RIGHT
            };
    ...
    if advanced_open {
        ...
        if let Some(buffer) = &mut app.provider_editor {
            buffer.open_advanced = editor.open_advanced;
        }
    }
        ...
        widgets::section(
            ...
            |ui| {
                ui.horizontal_wrapped(|ui| {
                    ...
                    if ui
                        .checkbox(
                            &mut editor.supports_websockets,
                            "支持 WebSocket (supports_websockets)",
                        )
                ...
                if widgets::field(
                    ...
                    |ui| {
                        widgets::opt_int_field(
                            ui,
                            "p-stream-retries",
                            &mut editor.stream_max_retries,
                        )
                    },
                ) {
                    changed = true;
                ...
                ui.add_space(4.0);
                ui.label(
                    RichText::new("自定义请求头 http_headers")
                        .size(12.5)
                        .strong()
                        .color(theme::TEXT),
                );
                widgets::hint(
                    ui,
                    "每次请求都会带上。有些中转站要求特定的 User-Agent 或自定义头。",
                );
                ...
                ui.add_space(6.0);
                ui.label(
                    RichText::new("从环境变量取请求头 env_http_headers")
                        .size(12.5)
                        .strong()
                        .color(theme::TEXT),
                );
                widgets::hint(
                    ui,
                    "值是环境变量名，运行时才取真值，适合放密钥类的 header。",
                );
                if widgets::kv_editor(
                    ui,
                    "p-env-headers",
                    &mut editor.env_http_headers,
                    "Header 名",
                    "环境变量名",
                ) {
                    changed = true;
