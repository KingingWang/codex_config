...
const KNOWN_FEATURES: &[(&str, &str, &str)] = &[
    ("hooks", "生命周期钩子", "在特定时机（回合开始/结束等）运行你自己的脚本。"),
    ("apply_patch_freeform", "自由格式补丁", "让模型用更宽松的补丁格式改文件，成功率更高。"),
    ("terminal_resize_reflow", "终端缩放重排", "终端窗口大小变化时重新排版已输出的内容。"),
    ("memories", "长期记忆", "跨会话记住你的偏好和项目信息。"),
    ("statistics", "使用统计", "在界面上显示 token 用量等统计信息。"),
    ("plugin_hooks", "插件钩子", "允许插件注册钩子。"),
    ("goals", "目标管理", "支持长任务的目标与预算跟踪。"),
    ("chronicle", "编年史", "记录会话时间线。"),
    ("js_repl", "JavaScript REPL", "允许模型运行 JavaScript 代码。"),
    ("code_mode", "代码模式", "让模型通过写代码来调用工具，而不是直接调用。"),
    ("context_management", "上下文管理", "自动裁剪上下文以节省 token。"),
    ("token_budget", "Token 预算", "支持给任务设置 token 预算上限。"),
    ("multi_agent_v2", "多代理 v2", "支持把任务拆分给子代理并行处理。"),
    ("guardianv2", "Guardian v2", "更细粒度的权限审查机制。"),
    ("network_proxy", "网络代理", "为子进程提供受控的网络代理。"),
    ("sleep_tool", "睡眠工具", "允许模型主动等待一段时间。"),
    ("current_time_reminder", "当前时间提醒", "在上下文里注入当前时间。"),
    ("rollout_budget", "会话用量预算", "限制单个会话的总用量。"),
    ("tool_registry", "工具注册表", "启用工具注册表配置。"),
];

const SESSION_PICKER_VIEWS: &[Choice] = &[
    Choice { value: "dense", label: "dense 紧凑（默认）", help: "会话列表更紧凑，一屏能看到更多。" },
    Choice { value: "comfortable", label: "comfortable 宽松", help: "每条会话占更多空间，更好读。" },
];
...
fn features_card(app: &mut App, ui: &mut Ui) {
    ...
    for (key, label, help) in KNOWN_FEATURES {
        keys.push(((*key).to_string(), (*label).to_string(), (*help).to_string()));
    }
    ...
    widgets::section(
        ...
        |ui| {
            widgets::hint(ui, "「默认」= 不写进配置文件，由 Codex 自己决定；「开」= true；「关」= false。");
            ui.add_space(8.0);
...
fn tui_card(app: &mut App, ui: &mut Ui) {
    widgets::section(
        ...
        |ui| {
            ...
            ui.add_space(6.0);
            settings::tri_row(app, ui, &["tui", "animations"], "动画效果", "欢迎页、加载动画等。关掉更省资源。");
            settings::tri_row(app, ui, &["tui", "whimsy"], "装饰特效", "输入框的小星星等装饰效果。");
            settings::tri_row(app, ui, &["tui", "show_tooltips"], "启动提示", "第一次进入时显示的小贴士。");
            settings::tri_row(app, ui, &["tui", "auto_recap"], "自动回顾", "终端失去焦点时自动生成对话回顾。");
            settings::tri_row(app, ui, &["tui", "vim_mode_default"], "默认 Vim 模式", "启动就用 Vim 键位编辑输入框。");
            settings::tri_row(app, ui, &["tui", "raw_output_mode"], "原始输出模式", "方便复制终端里的内容。");
        },
    );
}
...
fn multiline_setting(app: &mut App, ui: &mut Ui, path: &[&str], label: &str, key: &str, rows: usize) {
    ...
    if changed {
        if let Some(doc) = &mut app.doc {
            if value.trim().is_empty() {
                ...
                doc.config.set_value_at(path, TomlValue::from(value.clone()));
            }
        }
    }
}
...
fn mcp_card(app: &mut App, ui: &mut Ui) {
    ...
    widgets::section(
        ...
        |ui| {
            if names.is_empty() {
                widgets::hint(ui, "还没有配置任何 MCP 服务器。不需要的话可以完全忽略这一节。");
            }
            for name in &names {
            ...
            ui.horizontal(|ui| {
                ...
                if widgets::text_field_w(ui, "new-mcp-name", &mut buffer, "名字，例如 github", 260.0) {
                ...
                ui.add_enabled_ui(!id.is_empty() && !taken, |ui| {
                    if widgets::primary_button(ui, &format!("{} 添加 MCP 服务器", icons::ADD)).clicked() {
                        if let Some(doc) = &mut app.doc {
                            doc.config.ensure_table_at(&["mcp_servers", &id]);
                            doc.config
                                .set_value_at(&["mcp_servers", &id, "command"], TomlValue::from(""));
                            doc.config
                                .set_item_at(&["mcp_servers", &id, "args"], Item::Value(TomlValue::Array(Array::new())));
                        }
...
fn mcp_summary(app: &App, name: &str) -> String {
    let Some(doc) = &app.doc else { return String::new() };
    let command = doc
        .config
...
fn mcp_editor(app: &mut App, ui: &mut Ui, name: &str) {
    ...
    if widgets::field(
        ui,
        FieldSpec::new("环境变量", "env", "启动这个 MCP 服务器时要设置的环境变量，通常用来传 API Key。"),
        ...
        if let Some(doc) = &mut app.doc {
            ...
            if entries.is_empty() {
                ...
                for (k, v) in entries {
                    table.insert(k.trim(), TomlValue::from(v).into());
                }
                doc.config
    ...
    settings::tri_row(app, ui, &mcp_path(name, "enabled"), "启用", "关掉后 Codex 不会启动这个服务器。");
